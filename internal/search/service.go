...
// notesSubquery returns a sub-SELECT for notes.
...
// articlesSubquery returns a sub-SELECT for articles.
// $1 = search query string, $2 = ownerID.
const articlesSubquery = `
SELECT
    e.id,
    'article'                                                                              AS type,
    e.title,
...
// rssItemsSubquery returns a sub-SELECT for rss_items (owner-only; no visibility filter).
// $1 = search query string, $2 = ownerID.
const rssItemsSubquery = `
SELECT
    ri.id,
    'rss_item'                                                                           AS type,
    ri.title,
...
func Search(ctx context.Context, db *sql.DB, ownerID uuid.UUID, query string, filter string) ([]SearchResult, error) {
	...
	rows, err := db.QueryContext(ctx, sql, query, ownerID)
	if err != nil {
